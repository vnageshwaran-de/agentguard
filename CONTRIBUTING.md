...
* SDK-specific instrumentation helpers under `agentprdiff/providers/`.
...
* A new agent framework. `agentprdiff` intentionally does not care how your
  agent is built.
...
agentprdiff check  suite.py   # should exit 0
```

## PR checklist

* Tests pass locally (`pytest`).
* Public API changes are reflected in `src/agentprdiff/__init__.py` and the
