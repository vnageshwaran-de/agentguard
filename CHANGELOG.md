...
All notable changes to `agentguard` are documented in this file. Originally
...
- Baseline store (JSON files under `.agentguard/baselines/`) designed to be
...
- Trace diff engine producing a structured `TraceDelta` (assertion pass/fail
  ...
  change).
- CLI: `agentguard init`, `agentguard record`, `agentguard check`, `agentguard diff`.
